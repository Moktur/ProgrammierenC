...
/*
/*      Konzepte/       Lesen und Schreiben von Textdateien
/*      Sprachelemente: 
/*
/*      Aufgabenteil:   nein
/*
/*      Beschreibung:   Das Programm speichert alle beschriebenen Elemente
/*                      eines Vektors in eine Textdatei und liest sie
/*                      wieder ein.
/*
/*****************************************************************************/
...
void main(void)
{
    ...
    printf("----- Beispiel: 11-09, Personendatei verwalten ------------\n\n");
    printf("\nDas Programm verwaltet Personendaten in einer Textdatei.\n");
...
void SchreibeDatei(PERSON *vektor, char letzterEintrag)
{
    ...
    scanf_s("%s", dateiname, (int)sizeof(dateiname)-1);
	 ...
	 /* Oeffne Datei zum Schreiben im Textmodus */
    ...
    for (aktuell = vektor; aktuell - vektor < letzterEintrag; aktuell++)
    {
        fprintf(datei, "Nummer : %d\n", aktuell->nummer);
        fprintf(datei, "Name   : %s\n", aktuell->name);
        fprintf(datei, "Vorname: %s\n\n", aktuell->vorname);
    }
...
char LeseDatei(PERSON *vektor)
{
    ...
    scanf_s("%s", dateiname, (int)sizeof(dateiname)-1);
	 ...
	 /* Oeffne Datei zum Lesen im Textmodus */
    ...
    /* Lese Datei Element fuer Element in den Vetor ein bis das Ende der
       Datei erreicht ist */
    while (!feof(datei))
    {
        fscanf_s(datei, "Nummer : %d\n", &(aktuell->nummer));
        fscanf_s(datei, "Name   : %s\n", &(aktuell->name), (int)sizeof(aktuell->name)-1);
        fscanf_s(datei, "Vorname: %s\n\n", &(aktuell->vorname), (int)sizeof(aktuell->vorname)-1);
